...
select owner, constraint_name, constraint_type, status, validated from dba_constraints where owner='BTEST' and TABLE_NAME = 'acdrtest';  

 SELECT cols.table_name, cols.column_name, cols.position, cons.status, cons.owner
FROM all_constraints cons, all_cons_columns cols
WHERE cols.table_name = 'CARD_DETAILS'
...
desc BTEST.CARD_DETAILS;
desc acdrtest;

SELECT created
  FROM dba_objects
 WHERE object_name = CARD_DETAILS
   AND owner = BTEST
   AND object_type = 'TABLE'
   
   select * from dba_objects where owner = 'BTEST' ;
   
   select * from dba_objects where owner = 'BTEST' and object_name = 'CARD_DETAILS';
...
SELECT name, TO_CHAR(SYSDATE,'DD-MON-YY HH24:MI:SS AM') db_date  FROM v$database, dual;

SELECT to_char(DBTIMEZONE) FROM DUAL;